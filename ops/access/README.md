...
The helper calls `ssh-keygen.exe` and prompts normally. It refuses to overwrite an existing key unless `-Force` is supplied.

## Node snippets

Initial SSH snippets for MEL-P and the PE nodes live in `node-snippets/`. They intentionally contain placeholders for `admin`, `enable`, and public key material.
