...
> **Scope note (ADR-003 v1.1, 2026-06-14).** The commands below are the **W1 containerlab/FRR baseline** (`clab-aurora-*`, `vtysh`). The **built Region A is now a Cisco GNS3 core** mapped to the Melbourne/Sydney/Brisbane/Geelong POP overlay — for current Region A operations use `region-a-plan.md` §6 (bring-up waves), §7 (per-node smoke tests, IOS/IOS-XR `show` commands), and §8 (ops + MOP shape). A region-specific runbook split (Region A GNS3 + Region B CML) is a pending follow-up.
