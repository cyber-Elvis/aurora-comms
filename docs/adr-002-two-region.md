...
> **⚠ SUPERSEDED IN PART by ADR-003 v1.4 (2026-06-15).** Region A's *vendor stack* is now **Cisco**, not Nokia, and the current local placement is the Dell/PC2 ADL -> GEL -> MEL-PE1 -> MEL-P line. Brisbane/Sydney move to PC1 / Region B CML planning, with SYD retaining the IOS-XRv VPNv4/ROV edge role. Nokia SR OS/SR Linux are **archived** (license + recipe cold-stored, recoverable via git history). The region *structure* described below (single-homed CEs, tenant model, Internet-edge intent, national POP model, capability envelope) **still holds**. **Canonical Region A build = `region-a-plan.md` v2.5; rationale = `adr-003-revendor-cisco-region-a.md`.** The Nokia tables below are retained as the original decision record.
