...
## Current state — 2026-06-14: Cisco Region A + secure access foundation (ADR-003 / ADR-004)
...
- **Secure access model (ADR-004):** two rings are now the target — Tailscale management ring for PC1/PC2/DO/Oracle hosts, and a virtual-edge WireGuard data-plane ring for lab transport. `admin` remains Elvis-owned break-glass; `aurora-codex` and `aurora-claude` are lab-node-only automation accounts. Host OSes must not appear as routed lab nodes.
...
### Secure access / SSH status — 2026-06-14

ADR-004 has been implemented in the repo as **design + operator tooling**, but not yet applied to live devices:
...
| `ops/access/aurora-ssh.ps1` | Done / tested dry-run | Renders SSH syntax for `mel-p1`, `mel-pe1`, `bne-pe1`, `syd-pe1`; supports `modern`, `sros-legacy`, and `network-console` profiles |
| `ops/access/inventory.yml` | Done | Non-secret aliases and endpoint metadata for MEL/PE nodes plus future ring edges |
| `ops/access/new-agent-key.ps1` | Done / not executed | Generates per-agent ed25519 keys under `%USERPROFILE%\.ssh`; no keys have been generated by Codex |
| `ops/access/node-snippets/` | Done / staged | Apply-ready placeholder snippets for MEL-P, MEL-PE1, BNE-PE1, and SYD-PE1 SSH access |
| `aurora-codex` / `aurora-claude` accounts | Not yet applied | Accounts are planned lab-node-only identities; no live-node config has been pushed yet |
| Live SSH to MEL-P / MEL-PE nodes | Not verified | Current Codex session could not reach Dell/GNS3 over `192.168.200.2:3080`, `100.118.0.46:22`, or node management IPs, so live application waits for Dell/GNS3 reachability |
| Secrets / private keys in repo | Clean | Snippets contain placeholders only; private keys, passwords, `secret 9`, API tokens, and cloud secrets remain out of repo |

Immediate next operator action:

1. Bring Dell/GNS3 management reachability back online from PC1/Codex.
2. Generate local public keys when ready:
...
3. Paste the relevant `ops/access/node-snippets/` config into each node console, replacing only the placeholders.
4. Validate with:
...
### Documentation hygiene — 2026-06-14
