...
- **PC1/PC2 internet path update (2026-06-26):** the local `192.168.137.0/24` segment (PC2/Dell ICS gateway `192.168.137.1`) is now PC1's **backup** internet path, not its primary. PC1 now prefers its **own Wi-Fi** (Realtek 8822BE driver updated from the 2017 build to the 2024 build; the `Aurora-WiFi-Failover` watchdog pins the Wi-Fi default-route metric below Ethernet and fails over to ICS only when Wi-Fi loses internet). PC1 still holds a DHCP lease on this segment (currently `192.168.137.13`) as that failover path. ICS is kept alive by PC2's health-gated `Aurora-ICS-Watchdog` (`ops/access/ics-watchdog.ps1`) after a silent ICS drop on 2026-06-26; both paths carry full ISP rate (~44/20 Mbps Opticomm). (Prior 2026-06-20 state: PC1 leased `.81` and preferred Ethernet over Wi-Fi.)
