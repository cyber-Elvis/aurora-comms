...
| POP | Active lab role |
| --- | --- |
| Melbourne | Core/transit hub: `Aurora-P` + `Aurora-PE-1`, Transit-A, logical Melbourne IXP attachment |
| Sydney | PC1 / Region B major interconnect and Region B/C handoff: `SYD-PE1`, IOS-XR, first ROV enforcer, logically reached from `MEL-P` |
...
The local Region A line is intentionally hosted on Dell/PC2 and is drawn geographically as `ADL-PE1 -> GEL-PE1 -> MEL-PE1 -> MEL-P`. `MEL-P` sits on the right as the local core and logical handoff toward PC1 / Region B `SYD-PE1`. Brisbane and Sydney are Region B nodes, not local Region A nodes.
...
- Cisco local P/PE core: ADL -> GEL -> MEL-PE1 -> MEL-P over IS-IS L2, LDP, iBGP VPNv4, MPLS L3VPN, with `MEL-P` as the right-side logical handoff toward PC1 / `SYD-PE1`.
...
- `docs/region-a-topology.svg` is the committed export; regenerate it from the Draw.io source after diagram-source changes.
- `docs/assets/topology-photos/generated/` contains the lab-specific photo cards used by the topology diagrams.
